  ...
  public function fetchdatapendaftarantakoorkk()
  {
    ...
    if (!empty($data)) {
      ...
      foreach ($data as $i) {
        ...
        $output .= '
        <th scope="row">' . ++$no . '</th>
        <td> <a href="' . base_url('users/viewdetail/') . encrypt_url($i['id']) . '" class="btn badge badge-secondary">Details</a></td>        
        <td>' . $i['name'] . '</td>
        <td>' . $i['nim'] . '</td>
        <td>' . $i['prodi'] . '</td>
        <td>' . substr($this->session->userdata('koordinator'), 6) . '</td>
        <td>' . $i['tahun'] . '</td>';
        if ($i['status_file'] == "Disetujui Ketua KK") {
          $output .= '<td><b>Disetujui</b></td>';
        } else {
          $output .= '<td id="action"> <a href="' . base_url('users/accketuakk/') . encrypt_url($i['id_guidance']) . '" class="btn badge badge-success">Acc</a>
              <a data-toggle="modal" data-target="#' . encrypt_url($i['id_tr']) . '" class="btn badge badge-danger" style="color: white;">Tolak</a></td>';
        }
        $output .= '</tr>';
      };
    } else {
